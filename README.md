...
A robotic hand that closes and opens based on how far your hand is from an ultrasonic sensor, so no physical contact with the controller is needed. Built around an Arduino Uno driving five servos through a tendon-driven InMoov-style hand kit, with a single HC-SR04 as the input.

The idea came from wanting to try a non-contact control scheme (inspired by sterile surgical interfaces where the operator cannot touch the tool). Most of the engineering work was not about the concept itself, but was mainly focused on making cheap hardware behave well: filtering out sensor noise, smoothing servo motion, and keeping the USB power rail from browning out when five servos move at once.
...
- **Non-blocking firmware.** Same story as my other projects - the current code uses `delay()` for the staggered actuation, which blocks the sensor loop. A `millis()`-based scheduler would also let the hand keep reading input while moving.
